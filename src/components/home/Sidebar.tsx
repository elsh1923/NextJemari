...
import { TrendingUp, Mail, ArrowRight, User as UserIcon } from "lucide-react";
...
export function Sidebar({ popularTags, popularAuthors = [] }: SidebarProps) {
  ...
  return (
    <aside className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2">
          {popularTags.slice(0, 10).map((tag, index) => (
            <Link
              key={tag.id}
              href={`/tag/${tag.slug}`}
              className="inline-block rounded-full bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700 transition-all duration-300 ease-in-out hover:bg-blue-100 hover:text-blue-700 hover:scale-110 hover:shadow-lg hover:-translate-y-0.5 dark:bg-[#1A1A1C] dark:text-slate-300 dark:hover:bg-blue-900/30 dark:hover:text-blue-300"
              style={{ animation: `slide-in-up 0.4s ease-out ${index * 0.05}s forwards`, opacity: 0 }}
            >
              {tag.name}
              <span className="ml-1.5 text-xs text-slate-500 transition-all duration-300 ease-in-out dark:text-slate-500">
                {tag._count.articles}
              </span>
            </Link>
          ))}
      ...
      <div className="group/newsletter opacity-0 overflow-hidden rounded-xl border border-slate-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-4 transition-all duration-500 hover:shadow-xl hover:scale-[1.02] hover:border-blue-200 dark:border-[#1A1A1C] dark:from-[#111113] dark:to-[#0A0A0C] dark:hover:border-blue-500/30 sm:p-6" style={{ animation: 'slide-in-right 0.6s ease-out 0.3s forwards' }}>
        <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600 transition-all duration-300 ease-in-out hover:scale-110 hover:rotate-6 hover:shadow-lg sm:mb-4 sm:h-12 sm:w-12 dark:bg-blue-500">
          <Mail className="h-5 w-5 text-white transition-all duration-300 ease-in-out sm:h-6 sm:w-6" />
        </div>
        ...
        <form onSubmit={handleSubscribe} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={loading}
            className="group/btn flex w-full items-center justify-center space-x-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-medium text-white transition-all duration-300 ease-in-out hover:bg-slate-800 hover:scale-105 hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
          >
            <span>{loading ? "Subscribing..." : "Subscribe"}</span>
            {!loading && <ArrowRight className="h-4 w-4 transition-transform duration-300 ease-in-out group-hover/btn:translate-x-1" />}
          </button>
        </form>
      </div>
          ...
          <div className="space-y-3">
            {popularAuthors.map((author, index) => (
              <Link
                ...
                className="group flex items-center space-x-3 rounded-lg p-2 transition-all duration-300 ease-in-out hover:bg-slate-50 hover:scale-105 dark:hover:bg-[#1A1A1C]"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate text-sm font-medium text-slate-900 transition-colors duration-300 ease-in-out group-hover:text-blue-600 dark:text-slate-100 dark:group-hover:text-blue-400">
                    {author.username}
                  </p>
                ...
                <ArrowRight className="h-4 w-4 flex-shrink-0 text-slate-400 opacity-0 transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:translate-x-1 dark:text-slate-500" />
              </Link>
            ))}
          </div>
        </div>
